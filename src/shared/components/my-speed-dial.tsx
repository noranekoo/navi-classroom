...
export default function SpeedDials(props: any) {
  const classes = useStyles();
  const [direction, setDirection] = React.useState<SpeedDialProps['direction']>('down');
  const [open, setOpen] = React.useState(false);
  const [hidden, setHidden] = React.useState(false);
  const actions = [
    { icon: props.pic !== '' ? props.pic: '', name: 'Thêm'+props.suffix },
    { icon: <SpellcheckIcon />, name: 'Chấm điểm' },
    { icon: <AssessmentIcon />, name: 'Bảng điểm' },
...
//   };
  ...
  return (
    ...
    <div className={classes.root}>
      {/* <FormControlLabel
        control={<Switch checked={hidden}  color="primary" />}
        label="Hidden"
      />
